function adjustAudioToExpectedSize(waveform, sampleRate = 22050 * 2, duration = 0.742) {
...
function getWaveformFromPreview(callback, audioPreviewUrl, sampleRate=22050) {
    ...
    wavesurfer.on('ready', () => wavesurfer.exportPCM(sampleRate).then(
        waveform => callback(waveform)
    ), sampleRate)
}
...
function encodeAudio(arangedSpectrogram) {
    ...
    let tensor = tf.tensor2d(arangedSpectrogram, [batchInputShape[1], batchInputShape[2]], 'float32');
    let [mu_tensor, log_variance_tensor] = encoderModel.predict(tf.reshape(tensor, shape = [1, batchInputShape[1], batchInputShape[2], 1]));
...
async function getUserUploadedModel() {
    ...
    const uploadDecoderWeights = document.getElementById('upload-decoder-weights');
    if (!uploadEncoderJson || !uploadEncoderWeights || !uploadDecoderJson || !uploadDecoderWeights) {
        logInfo("Failed to include user VAE!!")
        return
    }
    console.log(uploadDecoderWeights)
    encoderModel = await tf.loadLayersModel(tf.io.browserFiles(
        [uploadEncoderJson.files[0], ...uploadEncoderWeights.files]));
    logInfo("Loaded user encoder.")
    decoderModel = await tf.loadLayersModel(tf.io.browserFiles(
        [uploadDecoderJson.files[0], ...uploadDecoderWeights.files]));
...
function convertPredictedSpectrogramIntoAudio(callback, predictedSpec, type = "2D", framelength = 1024) {
    ...
    let signal = ispectrogramMCLT(callback, reconstructedSpecTransposed, framelength);
